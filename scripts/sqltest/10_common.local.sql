...
-- 이 SQL 이 **거절되어야** 한다는 것을 확인한다.
-- 제약을 적어 두고 실제로는 안 걸리는 경우가 있어서, 통과가 아니라
-- '막혔는가'를 봐야 한다.
create or replace function public._assert_raises(p_sql text, p_label text)
returns void language plpgsql set search_path = public as $fn$
begin
  begin
    execute p_sql;
  exception when others then
    raise notice '  OK   % (막힘: %)', p_label, left(SQLERRM, 60);
    return;
  end;
  raise exception 'FAIL  % — 막혔어야 하는데 통과했습니다', p_label;
end;
$fn$;
